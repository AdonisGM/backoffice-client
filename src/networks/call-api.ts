...
export const callApi = async <Req, Res>({
  ...
  const endpoint =
    import.meta.env.MODE === 'production' ? '/api' : (import.meta.env.VITE_APP_URL as string);
  ...
  const processBody =
    api.contentType === 'application/json' ? JSON.stringify(body) : (body as FormData);
...
export const callDownloadFile = async ({ api, params }: { api: BasicApi; params?: string[] }) => {
  ...
  const apiFileName = contentDisposition
